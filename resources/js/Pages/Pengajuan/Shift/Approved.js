...
export default function Approved({ errors, shift }) {

    const [values, setValues] = useState({
        nomor_surat: shift.nomor_surat,
        tanggal_surat: shift.tanggal_surat,
        komentar: shift.komentar,
        file: shift.file,
        id: shift.id,
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('pengajuan.shift.update'), values);
    }
        ...
        <div className="card mb-5 mb-xl-8">
            <div className="card-header border-0 pt-5 flex">
                <h3 className="card-title align-items-start flex-column">
                    <span className="card-label fw-bolder fs-3 mb-1">Form Terima perubahan shift</span>
                </h3>
                <div class="card-toolbar">
                    <Link href={route('pengajuan.shift.index')} class="btn btn-dark fw-bolder me-auto px-4 py-3">Kembali</Link>
                </div>
            </div>
