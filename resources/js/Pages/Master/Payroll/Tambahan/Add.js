...
export default function Add({ errors, tambahan }) {

    const [kode, setKode] = useState(tambahan.kode_persen ?? [])
    const [values, setValues] = useState({
        kode_tambah: tambahan.kode_tambah,
        nama: tambahan.nama,
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('master.payroll.penambahan.store'), values);
    }
            ...
            <div className="toolbar mb-5 mb-lg-7">
                <div className="page-title d-flex flex-column me-3">
                    ...
                    <ul className="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7 my-1">
                        ...
                        <li className="breadcrumb-item text-gray-600">Komponen Tambahan</li>
                        <li className="breadcrumb-item text-gray-500">Data</li>
                    </ul>
                </div>
                <div className="d-flex align-items-center py-2 py-md-1" >
                    <Link href={route('master.payroll.penambahan.index')} className="btn btn-dark"><b>Kembali</b></Link>
                </div>
            </div>
                ...
                <div className="card-body py-3">
                    <form onSubmit={submit}>
                        <div className="row mb-6">
                            <label className="col-lg-3 col-form-label required fw-bold fs-6">Kode</label>
                            <div className="col-lg-9 fv-row fv-plugins-icon-container">
                                <input name="kode_tambah" disabled={values.id != undefined ? true : false} type="number" onChange={updateData} value={values.kode_tambah} className="form-control form-control-lg form-control-solid" />
                            </div>
