...
(function($) {
    ...
    $.sessionTimeout = function(options) {
        ...
        if (typeof opt.onWarn !== 'function') {
            ...
            $('body').append('<div class="modal fade" data-bs-backdrop="static" id="session-timeout-dialog"> \
              <div class="modal-dialog modal-sm" > \
                <div class="modal-content px-2"> \
                  <div class="modal-body pt-5"> \
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true">&times;</button> \
                  <h4 class="mb-2">' + opt.title + '</h4> \
                  <p>' + opt.message + '</p> \
                    ' + countdownMessage + ' \
                    ' + coundownBarHtml + ' \
                  </div> \
                  <div class="modal-footer"> \
                    <button id="session-timeout-dialog-logout" type="button" class="btn btn-block btn-primary">' + opt.logoutButton + '</button> \
                    <button id="session-timeout-dialog-keepalive" type="button" class="btn btn-block btn-link" data-bs-dismiss="modal">' + opt.keepAliveButton + '</button> \
                  </div> \
                </div> \
              </div> \
             </div>');
