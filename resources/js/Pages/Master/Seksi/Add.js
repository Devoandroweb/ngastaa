...
export default function Add({ errors, seksi }) {

    const [values, setValues] = useState({
        kode_seksi: seksi.kode_seksi,
        kode_skpd: seksi.kode_skpd,
        kode_bidang: seksi.kode_bidang,
        nama: seksi.nama,
        singkatan: seksi.singkatan,
        id: seksi.id,
    })
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('master.seksi.store'), values);
    }
        ...
        <div>
             <div className="toolbar mb-5 mb-lg-7">
                ...
                <div className="d-flex align-items-center py-2 py-md-1" >
                    <Link href={route('master.seksi.index')} className="btn btn-dark"><b>Kembali</b></Link>
                </div>
            </div>
            <div className="card mb-5 mb-xl-8">
                <div className="card-header border-0 pt-5 flex">
                    <h3 className="card-title align-items-start flex-column">
                        <span className="card-label fw-bolder fs-3 mb-1">Tambah seksi</span>
                    </h3>
                </div>
                <div className="card-body py-3">
                    <form onSubmit={submit}>
                        ...
                        <div className="row mb-6">
                            <label className="col-lg-3 col-form-label required fw-bold fs-6">Kode seksi</label>
                            <div className="col-lg-9 fv-row fv-plugins-icon-container">
                                <input name="kode_seksi" required disabled={values.id != undefined ? true : false} type="number" onChange={updateData} value={values.kode_seksi} className="form-control form-control-lg form-control-solid" />
                            </div>
                            {errors.kode_seksi && <div className="text-danger">{errors.kode_seksi}</div>}
                        </div>
                        <div className="row mb-6">
                            <label className="col-lg-3 col-form-label required fw-bold fs-6">Nama seksi</label>
                            <div className="col-lg-9 fv-row fv-plugins-icon-container">
