...
export default function Add({ errors, pendidikan }) {

    const [values, setValues] = useState({
        kode_pendidikan: pendidikan.kode_pendidikan,
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('master.pendidikan.store'), values);
    }
             ...
             <div className="toolbar mb-5 mb-lg-7">
                <div className="page-title d-flex flex-column me-3">
                    ...
                    <ul className="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7 my-1">
                        ...
                        <li className="breadcrumb-item text-gray-600">Pendidikan</li>
                        <li className="breadcrumb-item text-gray-500">Data</li>
                    </ul>
                </div>
                <div className="d-flex align-items-center py-2 py-md-1" >
                    <Link href={route('master.pendidikan.index')} className="btn btn-dark"><b>Kembali</b></Link>
                </div>
            </div>
            <div className="card mb-5 mb-xl-8">
                <div className="card-header border-0 pt-5 flex">
                    <h3 className="card-title align-items-start flex-column">
                        <span className="card-label fw-bolder fs-3 mb-1">Tambah Pendidikan</span>
                    </h3>
                </div>
                <div className="card-body py-3">
                    <form onSubmit={submit}>
                        <div className="row mb-6">
                            <label className="col-lg-3 col-form-label required fw-bold fs-6">Kode Pendidikan</label>
                            <div className="col-lg-9 fv-row fv-plugins-icon-container">
                                <input name="kode_pendidikan" disabled={values.id != undefined ? true : false} type="number" onChange={updateData} value={values.kode_pendidikan} className="form-control form-control-lg form-control-solid" />
                            </div>
