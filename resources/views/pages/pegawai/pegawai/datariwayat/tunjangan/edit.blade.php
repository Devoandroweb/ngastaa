...
@endif
<form class="edit-post-form" action="{{url('pegawai/jabatan/edit'.$pegawi->nip)}}" method="post">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="form-group">
                <select class="form-control select2" name="jenis_tunjangan" required>
                    <option selected disabled>Select Jenis Tunjangan</option>
                    <option selected >Select Jenis Tunjangan</option>
                    @foreach($pegawai as $s)
                        <option value="{{$s->kode_tunjangan}}">{{$s->status}}</option>
                    @endforeach
                </select>
                <div class="invalid-feedback">
                    {{ $errors->first('jenis_tunjangan') }}
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="form-group">
                <input class="form-control mb-3 @error('nominal') is-invalid @enderror" placeholder="Masukkan Nominal" name="nominal" type="number" value="{{$pegawai->nominal}}">
                <div class="invalid-feedback">
