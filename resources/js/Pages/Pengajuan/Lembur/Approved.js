...
export default function Approved({ errors, lembur }) {

    const [values, setValues] = useState({
        nomor_surat: lembur.nomor_surat,
        tanggal_surat: lembur.tanggal_surat,
        komentar: lembur.komentar,
        file: lembur.file,
        id: lembur.id,
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('pengajuan.lembur.update'), values);
    }
        ...
        <div className="card mb-5 mb-xl-8">
            <div className="card-header border-0 pt-5 flex">
                <h3 className="card-title align-items-start flex-column">
                    <span className="card-label fw-bolder fs-3 mb-1">Form Terima Pengajuan Lembur</span>
                </h3>
                <div class="card-toolbar">
                    <Link href={route('pengajuan.lembur.index')} class="btn btn-dark fw-bolder me-auto px-4 py-3">Kembali</Link>
                </div>
            </div>
