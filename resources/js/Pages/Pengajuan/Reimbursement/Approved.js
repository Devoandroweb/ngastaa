...
export default function Approved({ errors, reimbursement }) {

    const [values, setValues] = useState({
        nomor_surat: reimbursement.nomor_surat,
        tanggal_surat: reimbursement.tanggal_surat,
        komentar: reimbursement.komentar,
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('pengajuan.reimbursement.update'), values);
    }
        ...
        <div className="card mb-5 mb-xl-8">
            <div className="card-header border-0 pt-5 flex">
                <h3 className="card-title align-items-start flex-column">
                    <span className="card-label fw-bolder fs-3 mb-1">Form Terima Pengajuan Reimbursement</span>
                </h3>
                <div class="card-toolbar">
                    <Link href={route('pengajuan.reimbursement.index')} class="btn btn-dark fw-bolder me-auto px-4 py-3">Kembali</Link>
                </div>
            </div>
